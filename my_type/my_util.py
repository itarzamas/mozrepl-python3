...
class _JsonEncoder(json.JSONEncoder):
    def __init__(self, *args, **kwargs):
        ...
        self.types = {'Raw': Raw, 'Object': Object}
...
def getCookiesFromHost(repl, host):
    ...
    repl._rawExecute("""
        ...
        null;
        """.format(
        iter=iter, baseVar=repl._baseVarname, host=convertToJs(host)))
    while repl.execute('{iter}.hasMoreElements()'.format(iter=iter)):
        buffer = repl.execute("""
            ...
            """.format(
            iter=iter, baseVar=repl._baseVarname))
        ...
        yield http.cookiejar.Cookie(
            0,
            cookie.get('name', None),
            cookie.get('value', None), None, False, domain,
            cookie.get('isDomain', None), initial_dot,
            cookie.get('path', None), False,
            cookie.get('isSecure', None), expires,
            cookie.get('isSession', None), None, None, {})
    pass
